...
test('isolated Cobra candidates are reviewable without replacing live results', async ({ page }) => {
  ...
  await input.fill('9')
  await page.getByRole('button', { name: '跳转', exact: true }).click()
  await page.getByRole('button', { name: '鲜明候选', exact: true }).click()
  await expect(page.getByText('Cobra 鲜明候选 · 不覆盖现有成品')).toBeVisible()
  const candidate = page.getByAltText('第 9 页鲜明上色候选')
